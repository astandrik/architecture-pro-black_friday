...
Параметр `maxStalenessSeconds` определяет максимально допустимое отставание secondary-узла от primary. Если secondary отстала дольше указанного порога, драйвер перестаёт отправлять на неё запросы и перенаправляет чтение на primary. Минимальное значение для шардированных кластеров — **90 секунд**.

Параметр задаётся в connection string приложения (строка подключения к MongoDB, которую приложение использует для создания клиента). В текущем проекте это переменная окружения `MONGODB_URL` в `compose.yaml`.
...
Connection string — это строка подключения к MongoDB, через которую приложение (PyMongo / Motor) получает адрес кластера и настройки по умолчанию. В проекте из задания 4 (`sharding-repl-cache`) это переменная `MONGODB_URL` в `compose.yaml`.
...
- `readPreference=secondaryPreferred` — по умолчанию чтение идёт на secondary-узлы (разгрузка primary)
- `maxStalenessSeconds=90` — если secondary отстала > 90 сек, чтение идёт на primary
- `readConcernLevel=local` — возвращает данные из локального хранилища узла (быстро, но без гарантии durability)

Операции, требующие строгой консистентности (статус заказа, остатки при checkout, корзина), переопределяют эти настройки на уровне конкретного запроса через API драйвера — устанавливая `readPreference=primary` и `readConcern=majority`.

#### Per-query override в MongoDB Shell

В MongoDB Shell `.readPref()` задаёт Read Preference для конкретного запроса, а `.readConcern()` — Read Concern. Параметр `maxStalenessSeconds` в shell не поддерживается — он действует только из connection string ([документация](https://www.mongodb.com/docs/manual/reference/method/cursor.readPref/)).
...
Из задания: при Black Friday (50 000 запросов/сек) магазин использовал MongoDB с Range-Based Sharding (шардирование на основе диапазонов значений шард-ключа). Когда нагрузка выросла и в кластер добавили новые шарды, MongoDB начала перемещать данные между всеми узлами, чтобы заново разбить диапазоны. Это перемещение заняло значительные ресурсы CPU и сети, из-за чего время отклика на запросы выросло в пик нагрузки.

В Cassandra используется другой подход — consistent hashing (консистентное хэширование). Данные распределяются по кольцу хэш-значений. Каждый узел отвечает за свой участок кольца. При добавлении нового узла он забирает часть данных только у своих соседей по кольцу, а не у всех узлов. Например, при добавлении 4-го узла в кластер из 3 перемещается примерно 25% данных, а не 100%. Кроме того, каждый физический узел Cassandra делится на 256 виртуальных узлов (vnodes), которые распределены по кольцу равномерно — это предотвращает перекос данных.
...
`NetworkTopologyStrategy` — стратегия репликации Cassandra, которая размещает реплики в разных стойках внутри датацентра для отказоустойчивости. Replication Factor (RF) = 3 означает, что каждая запись хранится на 3 узлах в датацентре `dc1`. При падении одного узла данные доступны на двух оставшихся.
...
В MongoDB (задание 7) все корзины хранились в одной коллекции `carts`, и шардирование по `{user_id: "hashed"}` было невозможно: у гостевых корзин `user_id = null`, хэш от null одинаков для всех записей — все гостевые корзины попадали бы на один шард. Поэтому пришлось использовать `{_id: "hashed"}`.

В Cassandra корзины авторизованных (`carts_by_user`) и гостевых (`carts_by_session`) разнесены по отдельным таблицам. Cassandra не решает, в какую таблицу обращаться — это логика приложения: если запрос приходит от авторизованного пользователя (`user_id` известен), приложение выполняет запрос к `carts_by_user`; если от гостя (`session_id` известен) — к `carts_by_session`. В `carts_by_user` partition key = `user_id`, и он всегда заполнен (таблица содержит только корзины авторизованных). В `carts_by_session` partition key = `session_id`, и он тоже всегда заполнен. Null-значений в partition key нет ни в одной из таблиц.
